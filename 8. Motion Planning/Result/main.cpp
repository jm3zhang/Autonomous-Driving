      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          for (int i = 0; i < sensor_fusion.size(); i++) {
            ...
            // find of the current car lane, especically after lane change
            int old_car_lane = lane;
            ...
            if (current_car_lane == old_car_lane){
              if (check_car_s > car_s && (check_car_s - car_s) < 30){
                //ref_vel = 29.5;
                too_close = true;
              }
            }else if (current_car_lane == old_car_lane - 1) {
              // check if the left lane has cars
              if ((car_s - 30) < check_car_s && (car_s + 40) > check_car_s){
                left_lane_has_car = true;
              }
            }
            else if (current_car_lane == old_car_lane + 1) {
              // check if the right lane has cars
              if ((car_s - 30) < check_car_s && (car_s + 40) > check_car_s){
          ...
          // change car lane if the front car is too slow and there is no car on the lane that we want to switch to
          if (too_close) {
            // check left lane
            if (left_lane_has_car == false && lane != 0) {
              // left lane is safe
              // change to left line
              lane--;
            } else if (right_lane_has_car  == false && lane != 2) {
              // right lane is safe
              // change to right line
              lane++;
            }
          } else {
            // if we are not in the middle lane, we should change back to middle for more lane change options
            if (lane != 1) {
              // check if it is safe to move back to middle from the left lane
              if (lane == 2 && left_lane_has_car == false) {
                lane = 1;
              }
              // check if it is safe to move back to middle from the right lane
              else if ((lane == 0 && right_lane_has_car == false)){
                lane = 1;
              }
            }
          }
